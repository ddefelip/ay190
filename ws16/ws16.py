...
class mydata:
    ...
    def setup_ID(self,rchange,gamma):
        ...
        rho1 = 1.0  # needed to change this and rho2 from given values
        rho2 = 0.25
        eps1 = 2.5
        eps2 = 1.795
...
def hlle(hyd):
    ...
    for i in range(1,hyd.n-2):
        ...
        fluxl[2,i] = hyd.velp[i]*(hyd.qp[2,i] + hyd.pressp[i])

        fluxr[0,i] = hyd.qm[0,i+1]*hyd.velm[i+1]
        fluxr[1,i] = hyd.qm[1,i+1]*hyd.velm[i+1] + hyd.pressm[i+1]
        fluxr[2,i] = hyd.velm[i+1]*(hyd.qm[2,i+1] + hyd.pressm[i+1])
...
while(t < tend):
    ...
    if(i % 10 == 0):
        ...
        mpl.title('Density Profile')
        mpl.xlabel('Position')
        mpl.ylabel('Density')
        mpl.draw()
        
        # plots for program output        
        #if i % 80 == 0:
        #    mpl.savefig('out_%i.pdf' % i)
        
        
        if float(timestring) == 0.2:            
            fname = "t=0.2_"+reconstruction_type+".dat" 
            outfile = open(fname,"w")
            for i in range(nzones):
                outstring = "%15.6E %15.6E %15.6E %15.6E \n" % \
                        (hyd.x[i],hyd.rho[i],hyd.press[i],hyd.vel[i])
                outfile.write(outstring)
            outfile.close()
            mpl.savefig('t=0.2_'+reconstruction_type+'.pdf')
